...
def test_get_all_users_usecases(client, email, password, expect_success):
    ...
    if expect_success:
        ...
        assert response.status_code == 200, "Expected successful companies fetch with token"
        ...
        assert response.status_code == 200, "Expected successful companies fetch without token"
        data = response.json()
        assert "result" in data
        assert "pagination" in data
        assert isinstance(data["result"], list)
